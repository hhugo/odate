...
module Make(Implem : Implem)(D : Duration.S) = struct
  ...
  module From = struct
    ...
    let parse_num ?max ?(min=1) =
      let check = match max with
        | Some ma ->
          (fun l c -> l < ma && c >= '0' && c <= '9' )
        | None ->
          (fun _ c -> c >= '0' && c <= '9' ) in
      let rec loop endpos pos l str =
        if pos < endpos && check l str.[pos]
        then loop endpos (pos+1) (l+1) str
        else l in
      (fun ptr ->
    ...
    let parse_pm =
      parse_sum [
        parse_constant_i "am" >>=| false;
        parse_constant_i "pm" >>=| true;
      ]
    ...
    let parse_directive,add_abbr =
      let l = ref [
          'a', parse_weekday true >>== (fun wday -> (fun d -> {d with wday }));
          'A', parse_weekday false >>== (fun wday -> (fun d -> {d with wday }));
          'b', parse_month true >>== (fun month -> (fun d -> {d with month }));
          'B', parse_month false >>== (fun month -> (fun d -> {d with month }));
          'd', parse_num ~max:2 >>== (fun day -> (fun d -> {d with day}));
          'H', parse_num ~max:2 >>== (fun h -> (fun d -> {d with h}));
          'I', parse_num ~max:2 >>== (fun h -> (fun d -> {d with h}));
          'k', parse_num ~max:2 >>== (fun h -> (fun d -> {d with h}));
          'l', parse_num ~max:2 >>== (fun h -> (fun d -> {d with h}));
          'm', parse_num ~max:2 >>== (fun month -> let month = Month.of_int month in (fun d -> {d with month}));
          'M', parse_num ~max:2 >>== (fun m -> (fun d -> {d with m}));
          'p', parse_pm >>== update_pm;
          'P', parse_pm >>== update_pm;
          'S', parse_num ~max:2 >>== (fun s -> (fun d -> {d with s}));
          'u', parse_num >>== (fun wday ->
            let wday = Weekday.of_int (wday -1) in (fun d -> {d with wday} ) );
          'w', parse_num >>== (fun wday ->
            let wday = Weekday.of_int wday in (fun d -> {d with wday} ) );
          'y', parse_num ~max:2 >>== (fun y ->
            let year = if y < 70 then 2000 + y else 1900 + y in
            (fun d -> {d with year }));
          'Y', parse_num ~max:4 >>== (fun year ->
            (fun d -> {d with year }));
          'z', parse_timezone >>== (fun tz -> (fun d -> {d with tz }) );
        ] in
      (fun (_,d) -> List.assoc d !l),(fun abbr -> let l' = !l in l:=abbr::l' )
    ...
    let generate_parser s =
      ...
      try
        ...
        let rec loop f = function
          | [] -> f
          | x::xs -> loop (fun ptr d' -> let d = x ptr d' in f ptr d ) xs
        in
    ...
    let human ?tz {s;m;h;day;month;year;tz=tz'} =
  ...
  module To = struct
    ...
    let parse_directive,add_abbr =
      let l = ref [
          ...
          ('E', false, (fun (_, d) -> match d.day with | 1 -> "1st" | 2 -> "2nd" | 3 -> "3rd" | x -> string_of_int x  ^ "th"));
          ('H', true,  (fun (p, d) -> pad(d.h, p, '0', 2)));
          ('I', true,  (fun (p, d) -> pad(Hour.convert_24h_to_12h d.h, p, '0', 2)));
          ('k', true,  (fun (p, d) -> pad(d.h, p, '_', 2)));
          ('l', true,  (fun (p, d) -> pad(Hour.convert_24h_to_12h d.h, p, '_', 2)));
      ...
      (fun c -> assoc c !l),(fun abbr -> let l' = !l in l:= abbr::l')
  ...
  let get_age d =
    ...
    let y = hn.year - h.year - (if d' < now then 0 else 1)
    in y
  ...
  module Printer = struct
    let combine p1 p2 = (fun d ->
      let s1 = p1 d
      and s2 = p2 d
      in
      s1 ^ s2)
    let generate_or_exit s = match To.generate_printer s with
      | Some p -> p
  ...
  module Parser = struct
    let combine p1 p2 = (fun ptr d ->
      let d = p1 ptr d in
      p2 ptr d)
    let sum = From.parse_sum
    let generate_or_exit s = match From.generate_parser s with
      | Some p -> p
